...
void * print_hello(void *arg) {
   uint64_t *args; //identificador da thread na aplicacao e numero de threads
   args = (uint64_t *) arg; //typecasting de ponteiros
...
int main(int argc, char* argv[]) {
   ...
   for(long int i=0; i<nthreads; i++) {
      ...
      uint64_t *args = (uint64_t *) malloc(sizeof(uint64_t) * 2);
      if(args==NULL){
         printf("ERRO de alocacao de memoria\n");
         return 2;
      }   
